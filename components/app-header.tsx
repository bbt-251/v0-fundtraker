...
export function AppHeader() {
    ...
    return (
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b bg-white dark:bg-gray-800 px-4 md:px-6">
                            ...
                            {getNavItems(userProfile?.role).map((item) => (
                                <div key={item.title} className="space-y-1">
                                    {item.children ? (
                                                ...
                                                <div className="ml-6 space-y-1">
                                                    {item.children.map((child) => (
                                                        <Link
                                                            key={child.href}
                                                            href={child.href}
                                                            className={`flex items-center rounded-md px-3 py-2 text-sm font-medium ${pathname === child.href || pathname.startsWith(`${child.href}/`)
                                        ...
                                        <Link
                                            href={item.href}
                                            className={`flex items-center rounded-md px-3 py-2 text-sm font-medium ${pathname === item.href || pathname.startsWith(`${item.href}/`)
                                                ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
            ...
            <div className="flex items-center">
                {userProfile?.role === "Project Owner" && !pathname.includes("createNewProject") && (
                    <Link
