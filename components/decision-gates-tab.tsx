...
import { addDecisionGate, deleteDecisionGate, getProject, updateDecisionGate } from "@/services/project-service"
...
import { DatePicker } from "./ui/ant-date-picker"
...
export function DecisionGatesTab({ projectId }: DecisionGatesTabProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const formattedDate = dayjs(dateTime).format("MMMM DD, YYYY hh:mm A");
            ...
            if (selectedGate) {
                ...
                await updateDecisionGate(projectId, selectedGate.id, {
                    name,
                    videoConferenceLink,
                    dateTime: formattedDate, // Save formatted date
                    objective,
                    participants,
                });
            } else {
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="grid gap-2">
                            ...
                            <DatePicker
                                showTime
                                value={dateTime ? dayjs(dateTime).toDate() : null}
                                onChange={(value) => setDateTime(value ? value.toISOString() : "")}
                        ...
                        <Button onClick={handleSubmit} disabled={isSubmitting}>
                            {isSubmitting ? (
                                <span className="flex items-center gap-2">
                                    <LoadingOutlined />
                                    {selectedGate ? "Updating..." : "Adding..."}
                                </span>
                            ) : (
                                selectedGate ? "Update Decision Gate" : "Add Decision Gate"
                            )}
                        </Button>
                    </DialogFooter>
                </DialogContent>
