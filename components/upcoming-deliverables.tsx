...
export function UpcomingDeliverables({ projects, selectedDate = new Date() }: UpcomingDeliverablesProps) {
  ...
  const findRelatedTasks = (deliverable: ProjectDeliverable & { projectName: string; projectId: string }) => {
    ...
    // Find tasks that belong to the dependent activities
    const dependentTasks = allProjectTasks.filter((task) => {
      // Check if the task belongs to any of the dependent activities
      return dependentActivities.some((activity) => {
        // If the activity has a tasks array, check if the task ID is in it
        if (activity.tasks && activity.tasks.includes(task.id)) {
          return true
        }

        // If the task has an activityId field, check if it matches the activity
  ...
  const getActivityNames = (deliverable: ProjectDeliverable & { projectId: string }) => {
    ...
    // Get activities from this project
